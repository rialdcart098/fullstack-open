...
const Navbar = () => {
  ...
  const handleLogOut = () => {
    dispatch(logOut());
    dispatch(
      setNotification({ message: "Logged out successfully", good: true }, 5),
    );
  };
  return (
    <nav className="row-span-1 h-full">
      <div className="bg-gray-950 grid grid-cols-4 h-full p-1 gap-3 place-items-center border-b-1 border-b-blue-300">
        <Link
          to="/"
          className="hover:text-blue-300 transition-all ease-in-out text-blue-200 p-3 w-full text-center text-4xl font-black font-momo-trust-display"
        >
          Blogs
        </Link>
        <Link
        ...
        {user && (
          <>
            ...
            <div className="col-span-1 flex justify-end items-center gap-3">
              <p className="text-blue-300 font-medium select-none">
                {user.name} logged in
              </p>
              <button
