...
const BlogPage = () => {
  ...
  const handleRemove = () => {
    if (window.confirm(`Delete ${blog.title}? Can't be reversed.`)) {
      ...
      dispatch(
        setNotification(
          { message: "Successfully deleted blog", good: true },
          5,
        ),
      );
      navigate("/");
  ...
  const handleLike = () => {
    ...
    dispatch(
      setNotification({ message: `You liked ${blog.title}`, good: true }, 5),
    );
  };
  return (
    <div className="font-momo-trust-display grid w-full h-full grid-cols-2 grid-rows-[0.1fr_0.25fr_auto] overflow-hidden">
      ...
      <div className="row-start-2 row-end-3">
        <p className="m-2 text-2xl text-blue-400">added by {blog.user.name}</p>
        <a
          href={blog.url}
      ...
      <div className="text-2xl text-blue-400 font-medium row-start-3 row-end-4 col-span-1">
        Likes:
        <span className="likes-value mx-2 text-3xl">{blog.likes}</span>
        {user && (
          <button
            onClick={handleLike}
      ...
      <div className="flex flex-col bg-gray-700 col-start-2 col-end-3 row-start-2 row-end-4 p-3 min-h-0">
        ...
        <ul className="flex-1 overflow-y-auto min-h-0 space-y-1 scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-700">
          {[...blog.comments].map((c) => (
            <li
              key={c}
              className="odd:bg-gray-700 even:bg-gray-600 px-2 py-1 rounded"
            >
              {c}
            </li>
          ))}
